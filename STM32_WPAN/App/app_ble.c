 ...
 */
#if (P2P_SERVER1 != 0)
/* 'FRNDS-' + 4 hex digits of the silicon UDN, filled in by BuildLocalName()
   before the first advertisement. One build serves every board. */
static char a_LocalName[] = {AD_TYPE_COMPLETE_LOCAL_NAME, 'F','R','N','D','S','-','0','0','0','0'};
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          /* Ask for ATT_MTU = CFG_BLE_MAX_ATT_MTU instead of waiting for the
           * central to. Until an exchange happens the link stays at the default
           * 23, which caps a notification at 20 bytes and truncates the 26-byte
           * mag packet (see Core/Inc/ble_notify_len.h). */
          if (aci_gatt_exchange_config(p_connection_complete_event->Connection_Handle) != BLE_STATUS_SUCCESS)
          {
            APP_DBG_MSG("  Fail   : aci_gatt_exchange_config\n");
          }
...
/* Stamp the low 16 bits of the unique device number into the advertised name. */
static void BuildLocalName(void)
{
  static const char hex[] = "0123456789ABCDEF";
  uint32_t udn = LL_FLASH_GetUDN();

  a_LocalName[7]  = hex[(udn >> 12) & 0xFU];
  a_LocalName[8]  = hex[(udn >>  8) & 0xFU];
  a_LocalName[9]  = hex[(udn >>  4) & 0xFU];
  a_LocalName[10] = hex[ udn        & 0xFU];
}


static void Adv_Request(APP_BLE_ConnStatus_t NewStatus)
{
  BuildLocalName(); // Get unique device name for 
  tBleStatus ret = BLE_STATUS_INVALID_PARAMS;
